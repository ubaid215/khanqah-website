...
const CoursesPage = () => {
  ...
  const debugApiResponse = (response: any) => {
    ...
    if (response?.data) {
      console.log("Data structure:", {
        ...
        arrayLength: Array.isArray(response.data.data)
  ...
  useEffect(() => {
    const fetchCategories = async () => {
      try {
        const response =
          (await apiClient.getCategories()) as unknown as ApiResponse;
  ...
  useEffect(() => {
    const fetchCourses = async () => {
      ...
      try {
        ...
        // For development: set some mock data for testing
        if (process.env.NODE_ENV === "development") {
          console.log("Using mock data for development");

          // Create a minimal mock course that satisfies the type requirements
          const mockCourse: any = {
            ...
            // Required properties with minimal values
            categoryNames: ["Arabic"],
            avgRating: 4.8,
            reviewCount: 45,
            studentCount: 150,
            instructor: {
          ...
          setCourses([mockCourse as CourseWithExtras]);
    ...
    const matchesCategory =
      ...
      (course.categories &&
        course.categories.some(
          (cat: any) => cat.category && cat.category.name === selectedCategory
        ));
  ...
  const sortedCourses = [...filteredCourses].sort(
    (a: CourseWithExtras, b: CourseWithExtras) => {
      switch (sortBy) {
        case "popular":
  ...
  const featuredCourses = sortedCourses
    .filter((c: CourseWithExtras) => c.featured)
    .slice(0, 3);
        ...
        <div className="relative h-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex flex-col justify-center">
          ...
          >
            ...
            <p className="text-base sm:text-lg lg:text-xl text-white/95 mb-6 sm:mb-8 max-w-3xl mx-auto drop-shadow-md">
              Learn from expert scholars and deepen your understanding of Islam
        ...
        {!loading && !error && sortedCourses.length === 0 && (
          ...
          >
            ...
            <p className="text-gray-600 mb-6 max-w-md mx-auto">
              Stay tuned! We're updating our course catalog with exciting new
              content.
            </p>
            {(searchQuery ||
              selectedCategory !== "All" ||
